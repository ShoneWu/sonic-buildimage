...
#define FPGA_DRIVER_VERSION "0.0.e"
...
struct fpga_pci_device *g_fpga_dev = NULL;
...
long fpga_ioctl(struct file *filp, unsigned int cmd, unsigned long user_addr)
{
    ...
    if (cmd == FPGA_READ) 
    {
		...
		I2C_DEBUG( "[%s] Read channel:%d addr:0x%x offset:0x%x length:%d \n", __func__, acc.channel, acc.addr, acc.offset, acc.length);
    ...
    else if (cmd == FPGA_WRITE) 
    {
        ...
        if (retval)
        {
			I2C_DEBUG("copy_from user retval:%d ",retval);
            return(-EFAULT);
        }

        if (acc.length + acc.offset > MAX_EEPROM_SIZE)
        {
			I2C_DEBUG(" acc.length + acc.offset > MAX_EEPROM_SIZE ");
            return(-EINVAL);
        }
		I2C_DEBUG( "[%s] Write channel:%d addr:0x%x offset:0x%x length:%d \n", __func__, acc.channel, acc.addr, acc.offset, acc.length);
		
        /* 
        According to the data length:
        1. If the data length is not larger than MAX_BUFF_SIZE, acc.buff will store the raw data.
        2. If the data length is larger than MAX_BUFF_SIZE, acc.buff will store the user-space address of the data buffer.
        */
        
        if(acc.length <= MAX_BUFF_SIZE)
        ...
        else
        {
            ...
            // Get the user-space address of the data buffer.
            memcpy(&ptr, acc.buff, sizeof(ptr));
          
            temp = kzalloc((acc.length * sizeof(unsigned char)), GFP_KERNEL);
            if (!temp)
                return(-ENOMEM);
            
            // Get the data from the user-space address of the data buffer.
            retval = copy_from_user((void *)temp, (const void *)ptr, acc.length);
            if (retval)
            {
                kfree(temp);
                return(-EFAULT);
            ...
            retval = fpga_smbus_write(fpga_pci_dev, acc.channel, acc.addr, acc.offset, acc.length, temp);
            kfree(temp);
        }
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    /* Register fpga global hooks */
    g_fpga_dev = fpga_pci_dev;
...
/*******************************************************************************
* DESCRIPTION:
*   trigger baresync reset signal by fpga
* INPUTS:
*   filp: fpga file operation
* OUTPUTS:
*   none
* RETURNS:
*   OK,  success
*   <0,  fail
*******************************************************************************/
int fpga_trigger_baresync_reset(void)
{
    struct fpga_pci_device *fpga_pci_dev = g_fpga_dev;
    if(fpga_pci_dev == NULL)
    {
        return -1;
    }
    // trigger 0x70 to send out BARE_SYNC RESET signal
    FPGA_ACCESS(*((unsigned int *) (fpga_pci_dev->fpga_base + 0x70)) = 0x1;)

    // recover 0x70 to 0
    FPGA_ACCESS(*((unsigned int *) (fpga_pci_dev->fpga_base + 0x70)) = 0x0;)

    return 0;
}
EXPORT_SYMBOL_GPL(fpga_trigger_baresync_reset);

/*******************************************************************************
* DESCRIPTION:
*   set specified FPGA chip register value
* INPUTS:
*   reg_offset: register offset, eg: 0x10
...
*   <0,  fail
*******************************************************************************/
int fpga_reg_set(int reg_offset, int reg_value)
{
    struct fpga_pci_device *fpga_pci_dev = g_fpga_dev;
    if(fpga_pci_dev == NULL)
    {
    ...
    FPGA_ACCESS(*((unsigned int *) (fpga_pci_dev->fpga_base + reg_offset)) = reg_value;);
    return 0;
}
EXPORT_SYMBOL_GPL(fpga_reg_set);

/*******************************************************************************
* DESCRIPTION:
*   get the value of register in specified FPGA chip
* INPUTS:
*   reg_offset: register offset, eg: 0x10
* OUTPUTS:
*   reg_value: register value
...
*   <0,  fail
*******************************************************************************/
int fpga_reg_get(int reg_offset, int *reg_value)
{
    struct fpga_pci_device *fpga_pci_dev = g_fpga_dev;
    if(fpga_pci_dev == NULL)
    {
    ...
    FPGA_ACCESS(*reg_value = ACCESS_ONCE(*((unsigned int *)(fpga_pci_dev->fpga_base + reg_offset))););
    return 0;
}
EXPORT_SYMBOL_GPL(fpga_reg_get);

/*******************************************************************************
* DESCRIPTION:
*   get the register value of i2c device by fpga smbus
* INPUTS:
*   acc_i2c: fpga smbus info
* OUTPUTS:
*   none
* RETURNS:
*   OK,  success
*   <0,  fail
*******************************************************************************/
int i2c_device_read_by_fpga_smbus(struct fpga_access acc_i2c)
{
    struct fpga_pci_device *fpga_pci_dev = g_fpga_dev;
    int ret = 0;
    ...
    ret = fpga_smbus_read(fpga_pci_dev, acc_i2c.channel, acc_i2c.addr, acc_i2c.offset, acc_i2c.length, acc_i2c.buff);
    return ret;
}
EXPORT_SYMBOL_GPL(i2c_device_read_by_fpga_smbus);

/*******************************************************************************
* DESCRIPTION:
*   set the register value of i2c device by fpga smbus
* INPUTS:
*   acc_i2c: fpga smbus info
* OUTPUTS:
*   none
...
*   <0,  fail
*******************************************************************************/
int i2c_device_write_by_fpga_smbus(struct fpga_access acc_i2c)
{
    struct fpga_pci_device *fpga_pci_dev = g_fpga_dev;
    int ret = 0;
    ...
    ret = fpga_smbus_write(fpga_pci_dev, acc_i2c.channel, acc_i2c.addr, acc_i2c.offset, acc_i2c.length, acc_i2c.buff);
    return ret;
}
EXPORT_SYMBOL_GPL(i2c_device_write_by_fpga_smbus);
