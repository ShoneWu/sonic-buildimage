#!/usr/bin/python
#
# Copyright (C) 2020 Accton Networks, Inc.
#
...
def print_eth_list():
    ...
    for i in range(eth_num):
        cmd = "ethtool -i eth%s | grep \"driver\|bus-info\"" % i
        print(cmd)
...
def diag_item_rename_eth():
    ...
    #eth_num = int(eth_num)
    eth_num = 7
    expect_driver=[I210_DRIVER, KR_DRIVER, LOM_DRIVER]
    target_driver_name = ''
    target_bus_major = ''
    target_bus_minor = ''
    compare_driver_name = ''
    ...
    for i in range(eth_num):
        for j in range(i, eth_num):
            if i == j:
                continue
            # get  eth target  one
            get_driver_cmd = "ethtool -i eth%s | grep driver" % i
            target_driver_name = exec_check_output_no_chk_retcode(get_driver_cmd).split(':')[1].strip()
            get_bus_cmd = "ethtool -i eth%s | grep bus-info" % i
            ...
            # get  eth compare one
            get_driver_cmd = "ethtool -i eth%s | grep driver" % j
            compare_driver_name = exec_check_output_no_chk_retcode(get_driver_cmd).split(':')[1].strip()
            get_bus_cmd = "ethtool -i eth%s | grep bus-info" % j
            bus_str_split = exec_check_output_no_chk_retcode(get_bus_cmd).split(':')
