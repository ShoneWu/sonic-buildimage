			 ...
			 char *buf)
{
	...
	switch(data->index) {
	/* Port 1-25 present status: read from i2c bus number '13'
		and CPLD slave address 0x61 */
	case as7946_74xkb_cpld1: status = as7946_74xkb_cpld_read(13, 0x61, reg);
		break;
	/* Port 26-50 present status: read from i2c bus number '14'
		and CPLD slave address 0x62 */
	case as7946_74xkb_cpld2: status = as7946_74xkb_cpld_read(14, 0x62, reg);
		break;
	/* Port 51-74 present status: read from i2c bus number '17'
		and CPLD slave address 0x63 */
	case as7946_74xkb_cpld3: status = as7946_74xkb_cpld_read(17, 0x63, reg);
		break;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	switch(data->index) {
	/* Port 11-25 present status: read from i2c bus number '13'
		and CPLD slave address 0x61 */
	case as7946_74xkb_cpld1:
		bus  = 13;
		addr = 0x61;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	switch(data->index) {
	/* Port 1-25 reset status: read from i2c bus number '13'
		and CPLD slave address 0x61 */
	case as7946_74xkb_cpld1:
		bus  = 13;
		addr = 0x61;
		break;
	/* Port 26-50 reset status: read from i2c bus number '14'
		and CPLD slave address 0x62 */
	case as7946_74xkb_cpld2:
		bus  = 14;
		addr = 0x62;
		break;
	/* Port 51-74 reset status: read from i2c bus number '17'
		and CPLD slave address 0x63 */
	case as7946_74xkb_cpld3:
		bus  = 17;
		addr = 0x63;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	switch(data->index) {
	case as7946_74xkb_cpld1: status = as7946_74xkb_cpld_write(13, 0x61, reg, val);
		break;
	case as7946_74xkb_cpld2: status = as7946_74xkb_cpld_write(14, 0x62, reg, val);
		break;
	case as7946_74xkb_cpld3: status = as7946_74xkb_cpld_write(17, 0x63, reg, val);
		break;
	default: status = -ENXIO;
			break;
	}
							...
							char *buf)
{
	...
	mutex_lock(&data->update_lock);
	switch(data->index) {
	case as7946_74xkb_cpld1: status = as7946_74xkb_cpld_read(13, 0x61, 0x1);
		break;
	case as7946_74xkb_cpld2: status = as7946_74xkb_cpld_read(14, 0x62, 0x1);
		break;
	case as7946_74xkb_cpld3: status = as7946_74xkb_cpld_read(17, 0x63, 0x1);
		break;
	default: status = -1;
			break;
	}
